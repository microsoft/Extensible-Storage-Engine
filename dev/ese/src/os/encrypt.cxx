...
#else
#define COMPUTE_CRC_SIZE 4
#endif

ULONG
ProcessorComputeCRC32(
...
)
{
    ...
    {
#if defined (_M_AMD64)
        crc = (ULONG) _mm_crc32_u64((unsigned __int64)crc, *(const unsigned __int64 UNALIGNED *)Buffer);
#else
        crc = _mm_crc32_u32(crc, *(const UINT UNALIGNED *)Buffer);
#endif
...
#endif // #if defined (_M_AMD64) || defined (_M_IX86)
    ...
    _Inout_                                             ULONG *pcbKeySize )
{
    ...
    BYTE* const pbData = ( cbKeySize != 0 ) ? pKey->pbKey : NULL;
    if ( !CryptExportKey( hKey, NULL, PLAINTEXTKEYBLOB, 0, pbData, &cbKeySize ) )
    {
    ...
    *pcbKeySize = cbKeySize + sizeof(AES256KEY);
    ...
    pKey->Version = JET_EncryptionAlgorithmAes256;
    pKey->Checksum = Crc32Checksum( pKey->pbKey, cbKeySize );
...
{
    BYTE    Version;
    BYTE    InitVector[BlockSizeAes256];
};
    ...
    _In_                                                    ULONG cbKey )
{
    ...
    *pcbDataLen += sizeof(checksum);
    ...
    if ( !CryptImportKey( g_hAESProv, pKey->pbKey, cbKey - sizeof(AES256KEY), NULL, 0, &hKey ) )
    {
        return ErrOSErrFromWin32Err(GetLastError());
    }
    ...
    if ( !CryptEncrypt( hKey, 0, TRUE, 0, pbData, pcbDataLen, cbDataBufLen ) )
    {
        Error( ErrOSErrFromWin32Err(GetLastError()) );
    }
    ...
    _In_                                                ULONG cbKey )
{
    ...
    *pcbDataLen -= sizeof(AES256BLOBTRAILER);
    ...
    memcpy( pbDataOut, pbDataIn, *pcbDataLen );
    if ( !CryptDecrypt( hKey, NULL, TRUE, 0, pbDataOut, pcbDataLen ) )
    {
